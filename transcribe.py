...
ARG_MUST_INCLUDE = ''     # default filter -> accept all mp3s
if len(sys.argv) > 1:
...
for talk in list_talks:
    ...
    path_vtt = path_transcript.replace('.txt', '.vtt')
    path_srt = path_transcript.replace('.txt', '.srt')
    print(f'removing {path_vtt}')
    print(f'removing {path_srt}')
